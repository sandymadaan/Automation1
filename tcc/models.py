...
'''class ClientJobForm(forms.ModelForm):
	...
	field = Field.objects.all()
        self.fields['material'] = forms.ModelChoiceField(queryset=Test.objects.filter(field_id=field_id))

    class Meta:
        model = ClientJob'''
		
#   field=forms.ModelChoiceField(Field.objects.all())
 #   test=forms.ModelChoiceField(Test.objects.none())

    #def __init__(self, *args, **kwargs):
    #    forms.Form.__init__(self, *args, **kwargs)
     #   field=Field.objects.all()
      #  if len(field)==1:
       #     self.fields['field'].initial=field[0].pk

        #field_id=self.fields['field'].initial or self.initial.get('field') \
         #         or self._raw_value('field')
       # if field_id:
            # parent is known. Now I can display the matching children.
        #    testz=test.objects.filter(field_id=field__id)
         #   self.fields['testz'].queryset=testz
          #  if len(testz)==1:
           #     self.fields['testz'].initial=testz[0].pk
