...
class UserProfileForm(ModelForm):
	class Meta :
		...
		widgets = {
            ...
            'address_1': AddressWithMapWidget({'class': 'vTextField'})
			}
...
class Job(models.Model):
	place =models.CharField(max_length=600, blank=True )
...
class ClientJobForm(forms.ModelForm):
	...
	class Meta :
		model = ClientJob
		exclude= ['client','job_no','material']
...
#	material = DynamicMultipleChoiceField(widget = forms.CheckboxSelectMultiple(), choices = [])
	...
	def filter_features(self, data):
		...
		query = data.get('filter_material_id', 0)

		if query:
			material = Material.objects.get(pk = query)
			tests = tests.filter(material_id = material)
